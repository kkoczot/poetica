...
function UserCarousel({ users, userId, follow }: { users: any[], userId: string, follow: "follow" | "unfollow" }) {
  ...
  return (
    <div className="flex gap-2">
      <button
        className={`text-white text-[24px] rounded-sm hover:bg-green-500/40 disabled:bg-green-700/15 ${current <= 1 ? "bg-green-700/15" : "bg-green-700/30"}`}
        disabled={current <= 1}
      ...
      <div key={users[current - 1]?.id} className="flex flex-col items-center flex-1">
        ...
        <span className="p-[2px] border border-transparent rounded-lg hover:border-white">
          <HandleFollowBtn authUserId={userId} userId={users[current - 1]?.id} follow={follow} />
        </span>
      </div>
      <button
        className={`text-white text-[24px] rounded-sm hover:bg-green-500/40 disabled:bg-green-700/15 ${current >= users.length ? "bg-green-700/15" : "bg-green-700/30"}`}
